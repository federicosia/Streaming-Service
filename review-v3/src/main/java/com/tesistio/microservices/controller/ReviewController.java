...
public class ReviewController {
    ...
    @Transactional
    @PatchMapping("/{id}/likes/increase")
    public ResponseEntity<Void> increaseLikes(@PathVariable long id) {
        service.addLikeToReview(id);

        return new ResponseEntity<>(HttpStatus.OK);
    }

    @Transactional
    @PatchMapping("/{id}/likes/remove")
    public ResponseEntity<Void> decreaseLikes(@PathVariable long id) {
        service.removeLikeToReview(id);

        return new ResponseEntity<>(HttpStatus.OK);
    }

    @GetMapping("/stars/{stars}")
    public ResponseEntity<List<Review>> getReviewByRating(@PathVariable int stars){
        return new ResponseEntity<>(service.findByRating(stars), HttpStatus.OK);
    }

    @GetMapping("/stars/{start}-{end}")
    public ResponseEntity<List<Review>> getReviewByRatingBetween(@PathVariable int start,
